...
train_dataset = datasets.DatasetFolder(root='data_processed_Att5Distance14cm/1000/training', loader=npy_load_file, extensions='npy')
test_dataset = datasets.DatasetFolder(root='data_processed_Att5Distance14cm/1000/testing', loader=npy_load_file, extensions='npy')
...
class NeuralNet(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes):
        ...
        self.layer1 = nn.BatchNorm1d(input_size) # a
        self.layer2 = nn.Linear(input_size, hidden_size) # b
        self.layer3 =nn.BatchNorm1d(hidden_size) # c
        self.layer4 = nn.ReLU() # d
        self.layer5 = nn.Linear(hidden_size, hidden_size) # e
        self.layer6 = nn.BatchNorm1d(hidden_size) # f
        self.layer7 = nn.ReLU() # g
        self.layer8 = nn.Linear(hidden_size, hidden_size) # h
        self.layer9 =nn.BatchNorm1d(hidden_size) # i
        self.layer10 =nn.ReLU() # j
        self.layer11 = nn.Linear(hidden_size, num_classes) # k
        self.layer12 =nn.BatchNorm1d(num_classes) # x
...
for epoch in range(num_epochs):
    ...
    for i, (data, labels) in enumerate(train_loader):
        ...
        # Forward pass
        outputs, _, _, _, _, _, _, _,_,_,_,_ = model(data.float())
...
# Forward pass
outputs, a, b, c, d, e, f, g, h, i ,j ,k = model(data_test.float())
...
torch.save(model.state_dict(), '/Users/mhni/Desktop/GOMX5-MARK3/NeuralNetwork/AMO/AMO_NN_Att5batchNorm.pkl')
test_data_output_num = outputs.detach().numpy()
a_num = a.detach().numpy()
b_num = b.detach().numpy()
c_num = c.detach().numpy()
d_num = d.detach().numpy()
e_num = e.detach().numpy()
f_num = f.detach().numpy()
g_num = g.detach().numpy()
h_num = h.detach().numpy()
i_num = i.detach().numpy()
j_num = j.detach().numpy()
k_num = k.detach().numpy()

test_data_output_filename = '/Users/mhni/Desktop/GOMX5-MARK3/NeuralNetwork/AMO/DNN_Output_TestData.mat'
sio.savemat(test_data_output_filename, {'DNN_Output': test_data_output_num, 'a': a_num, 'b': b_num, 'c': c_num, 'd': d_num, 'e': e_num, 'f': f_num, 'g': g_num,'h': h_num, 'i': i_num, 'j': j_num, 'k': k_num })
def plot_confusion_matrix(y_true, y_pred, classes, labels=None,
    ...
    print(cm)
    np.savetxt('confusionMat_Att5_batchNorm.txt', cm, delimiter=',')
    fig, ax = plt.subplots()
...
plot_confusion_matrix(labels_test, outputs.argmax(1).detach().numpy(), classes=classes_int, normalize='true',
                      labels=classes)
#plt.savefig('conf_mat_Att5Distance14cm_50epoch.pdf')

plt.figure()
plt.plot(loss_vec)
plt.xlabel('Number of training steps')
plt.ylabel('Cross entropy loss')
#plt.savefig('Loss_vec_Att5Distance14cm_50epoch.pdf')
