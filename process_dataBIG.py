...
num_samples = 1000
version = ''
folders = os.listdir('data_Reallybig')

IQ = np.zeros((49, 10, num_samples, 2, 5000))
for f_num, folder in enumerate(folders):
    for i in range(0, 10):
        if i in range(9):
            ...
            version = 'testing/'
        filename_load = 'data_Reallybig/' + folder + '/' + str(i + 1) + '.mat'
        print(filename_load)
        ...
        ##
        if not os.path.exists('data_processed_Reallybig/1000/' + version + folder):
            os.makedirs('data_processed_Reallybig/1000/' + version + folder)
    num = 0
    for i in range(0, 10):
        if i in range(9):
            version = 'training/'
        ...
        for k in range(IQ.shape[2]):
            ...
            filename_save = 'data_processed_Reallybig/1000/' + version + folder + '/' + str(num) + '.npy'
